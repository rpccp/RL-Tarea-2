...
{\large Resumen del capítulo 2\par}
\vspace{0.7em}
...
\vspace{0.35em}
{\small Basado en \textit{Reinforcement Learning: An Introduction}, de
Richard S. Sutton y Andrew G. Barto.\par}
\vspace{0.5em}
\hrule
\vspace{0.8em}
{\small\itshape Nota: no se incluye la parte de \textit{Gradient Bandits}.\par}
...
\concepto{\textbf{Exploración frente a explotación}}
...
Se buscan \textbf{métodos simples que consigan un buen equilibrio}.

Para \textbf{estimar cuánto vale cada acción}, la idea es hacer dos cosas:
...
\subsection{Promediar las recompensas}
...
Si la acción todavía nunca fue elegida, puede asignarse un valor inicial como
...
En palabras: la estimación del valor de la acción $a$ se aproxima cada vez más
a su valor verdadero a medida que aumenta la experiencia.

La estrategia más simple es elegir siempre la acción con mayor valor estimado. A esto se lo denomina
...
todas las acciones son probadas repetidamente. Por lo tanto,

\[
  N_t(a)\longrightarrow\infty,
\]

y las estimaciones terminan acercándose a sus valores verdaderos.

El libro compara estos algoritmos mediante un problema experimental con $n=10$
acciones. Los valores verdaderos $q(a)$ se generan con una distribución normal:

\[
  \mu=0,\qquad \sigma^2=1,
\]

y las recompensas también contienen ruido aleatorio. Los resultados se
promedian sobre \textbf{2000 problemas diferentes durante 1000 pasos}. Se comparan:
...
problemas \textbf{no estacionarios}, donde

\[
  q_t(a)
\]

puede cambiar con el tiempo. La expresión se lee ``$q$ sub $t$ de $a$'' y
significa el valor verdadero de la acción $a$ en el instante $t$. Una acción que
antes era mala puede convertirse posteriormente en la mejor. Por eso, el
equilibrio entre exploración y explotación es una característica fundamental de RL.
...
\subsection{Una regla que nos aparece}
...
\[
  \text{Error}=\text{Objetivo}-\text{Estimación anterior}.
\]
...
Hasta ahora veníamos suponiendo que $q(a)$ era constante. Pero en muchos entornos

\[
  q_t(a)\neq q_{t+1}(a).
\]

En esos casos, promediar \textbf{toda la historia} puede jugar en contra: las
recompensas de hace mucho tiempo quizás ya no representen lo que está pasando
ahora.

Para dar mayor importancia a las observaciones recientes se utiliza
...
antigua disminuye aproximadamente de forma exponencial:
...
convergencia suelen exigirse las condiciones
...
cumple ambas condiciones. En cambio, un valor constante

\[
  \alpha_k=\alpha
\]

no cumple la segunda. Esto \textbf{no es necesariamente malo}: en un problema
...
aunque las recompensas reales estén aproximadamente alrededor de cero. Esto se
denomina \textbf{valores iniciales optimistas}.
...
$N_t(a)$ indica cuántas veces elegimos la acción. Si fue elegida pocas veces,
$N_t(a)$ es pequeño y
...
es grande. Por lo tanto, UCB la considera atractiva para explorar. Cuando la
probamos muchas veces, $N_t(a)$ aumenta y el incentivo de exploración disminuye.

El parámetro $c>0$ controla cuánto se valora la exploración:
...
\subsection{La idea detrás de UCB}

UCB puede interpretarse como la elección de la acción con el mayor
...
tenemos que aprender una relación

\[
  \text{situación}\longrightarrow\text{acción},
\]

es decir, una \textbf{política}.
...
En RL completo, además,

\concepto{\textbf{la acción también afecta el estado futuro}.}

Nos quedamos con que \textbf{aprender no es solamente elegir lo que hoy parece mejor;
también hay que invertir algunas acciones en conseguir información para decidir
mejor más adelante}.
